...
// Helper to convert number to Indian words (Simple implementation)
...
export const generatePayslipPDF = (data) => {
  ...
  doc.autoTable({
    ...
    body: [
      ['Basic Pay', formatINR(data.basic_pay), 'PF (Employee)', formatINR(data.pf_employee)],
      ['HRA', formatINR(data.hra), 'ESI (Employee)', formatINR(data.esi_employee)],
      ['DA', formatINR(data.da), 'Professional Tax', formatINR(data.professional_tax)],
      ['Special Allowance', formatINR(data.special_allowance), 'TDS', formatINR(data.tds)],
      ['Overtime Pay', formatINR(data.overtime_pay), 'LOP Deduction', formatINR(data.lop_deduction)],
      ['Bonus', formatINR(data.bonus), '', ''],
      [{ content: 'Total Earnings', styles: { fontStyle: 'bold' } }, { content: formatINR(data.gross_salary), styles: { fontStyle: 'bold' } }, { content: 'Total Deductions', styles: { fontStyle: 'bold' } }, { content: formatINR(data.total_deductions), styles: { fontStyle: 'bold' } }]
    ],
    theme: 'grid',
    headStyles: { fillColor: [59, 130, 246] },
  });
  ...
  doc.text(`NET PAYABLE: ${formatINR(data.net_salary)}`, pageWidth - 20, y, { align: 'right' });
  
  doc.setFontSize(10);
  doc.setFont(undefined, 'italic');
  doc.text(`(Amount in words: Rupees ${numberToWords(data.net_salary)})`, 20, y + 8);
